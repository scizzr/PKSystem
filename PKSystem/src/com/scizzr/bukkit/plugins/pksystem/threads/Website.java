...
public class Website implements Runnable {
    ...
    public static void checkVersion(Player p) {
        try {
            URL url = new URL(
                String.format("http://www.scizzr.com/util/plugins/version.php?plug=" + Main.jar.split("\\.")[0])
            );
                ...
                if (!verNew.equalsIgnoreCase(verCur)) {
                    if (p != null) {
                        if (Vault.hasPermission(p, "newver")) {
                            p.sendMessage(Main.prefix + "Your version of " + Main.jar.split("\\.")[0] + " is out of date.");
                            p.sendMessage("Version " + verNew + " can be downloaded from this link:");
                            p.sendMessage(ChatColor.YELLOW + "http://www.scizzr.com/plugins/PKSystem/PKSystem.jar");
                        }
                    }
                }
                line = buff.readLine();
